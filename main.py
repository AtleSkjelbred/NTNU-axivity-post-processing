...
def main():
    ...
    for csvfile in glob.glob(data_path + '*.csv'):
        ...
        df = filter.filter_dataframe(new_line, df, epm, settings)
        index = get_index(df, settings['time_column'])
        filter.filter_days(df, index, settings)
        ...
        calculate_variables.calculate_variables(df, new_line, index, ot_index, date_info, variables, epm, epd, settings)
...
def get_variables(new_line, epm, df, index, date_info, ot_index, ot_date_info, settings):
    variables = {}
...
def epoch_test(new_line, df, time_column):
...
def get_index(df, time_column) -> dict:
